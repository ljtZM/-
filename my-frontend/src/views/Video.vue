...
export default {
  ...
  methods: {
    ...
    getVideoComments() {
      axios
        .get(`http://localhost:8081/api/getvideocomments?video_id=${this.id}`)
        .then((response) => {
    ...
    addVideoComments() {
      if (this.newComment.trim() !== '') {
        ...
        axios
          .get('http://localhost:8081/api/addvideocomments', {
            video_id: this.id,
    ...
    getVideoLikes() {
      axios
        .get(`http://localhost:8081/api/getvideolikes?video_id=${this.id}`)
        .then((response) => {
    ...
    likeVideo() {
      ...
      if (video) {
        ...
        axios
          .get('http://localhost:8081/api/addvideolikes?video_id=' + this.id)
          .then((response) => {
            console.log(response);
          })
          .catch((error) => {
